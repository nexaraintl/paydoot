...
(function ($) {
  ...
  function avatarUpload1() {
    let readURL = function (input) {
      if (input.files && input.files[0]) {
        let reader = new FileReader();
        reader.onload = function (e) {
          $(".avatrSrc1").attr("src", e.target.result);
        };

        reader.readAsDataURL(input.files[0]);
      }
    };
    $(".upload-avatar-input1").on("change", function () {
      readURL(this);
    });
  }

  $(".dm-upload-avatar1").on("click", function (e) {
    e.preventDefault();
    avatarUpload1();
    $(".upload-avatar-input1").click();
  });

  //upload aadhar card back-side
  function avatarUpload2() {
    let readURL = function (input) {
      if (input.files && input.files[0]) {
        let reader = new FileReader();
        reader.onload = function (e) {
          $(".avatrSrc2").attr("src", e.target.result);
        };

        reader.readAsDataURL(input.files[0]);
      }
    };
    $(".upload-avatar-input2").on("change", function () {
      readURL(this);
    });
  }

  $(".dm-upload-avatar2").on("click", function (e) {
    e.preventDefault();
    avatarUpload2();
    $(".upload-avatar-input2").click();
  });
